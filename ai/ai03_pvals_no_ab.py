"""This is a simple AI that uses Point Value as a heuristic to play
   Also, this version does not use Alpha-Beta Pruning.
...
class AI(ai_generic):
    ...
    def minimax(self, board, depth, player, argmove):
        """minimax without"""
        ...
        if player == 'w':
            ...
            for move in movelist:
                board.push(move)
                p_eval, _ = self.minimax(board, depth-1, 'b', move)
                board.pop()
                if p_eval > best_eval:
                    best_eval = p_eval
                    best_move = move
            ...
            for move in movelist:
                board.push(move)
                p_eval, _ = self.minimax(board, depth-1, 'w', move)
                board.pop()
                if p_eval < best_eval:
                    best_eval = p_eval
                    best_move = move
    ...
    def move(self, board, player):
        """deals with ai input"""
        final_eval, final_move = self.minimax(
            board, self.G_DEPTH, player, None)
        return final_move
